...
class VisibityDegreeDistribution {
  ...
  private var psvgResults:PSVGResults = _
  ...
  def getDegreeDistribution():Dataset[Row] = {
    degreeDistribution
  }
  private def calculateFractalDimension(degreeDistStart:Int,degreeDistDataPartFromStart:Double,includeIntercept:Boolean,rejectCut:Double): Unit = {
    ...
    val deg = degreeList.map(row => {
      new DegreeDist(row.getInt(0),row.getLong(1),row.getDouble(2),row.getDouble(3),row.getDouble(4))
    })
    ...
    psvgResults = new PSVGResults(regset.getSlope,regset.getIntercept,regset.getRSquare,regset.getSlopeStdErr,regset.getInterceptStdErr)

  }
  def getPSVGResults(): PSVGResults = {
    psvgResults
  }
